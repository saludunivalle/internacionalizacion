...
export const AuthProvider = ({ children }) => {
  ...
  const resolveRoleFromUsersSheet = useCallback(async (email, token) => {
    console.log(
      "Resolviendo rol desde la hoja de usuarios para el email:",
      email,
    );
    const sheetMap = await apiGetAllSheets(token);
    const rows = getSheetRows(sheetMap, "USUARIOS", ["usuarios", "users"]);
    console.log("Filas obtenidas de la hoja de usuarios:", rows);
    const matchedUser = rows.find((row) => {
      const rowEmail = pickValue(row, ["correo", "email"], 1);
      return (
        String(rowEmail ?? "").toLowerCase() ===
        String(email ?? "").toLowerCase()
      );
    });
    console.log("Usuario encontrado en la hoja de usuarios:", matchedUser);
    if (!matchedUser) {
      return {
        userId: null,
        role: "",
    ...
    console.log("Usuario autenticado en la hoja:", matchedUser);
    return {
  ...
  const loginWithGoogleCredential = useCallback(
    async (credential) => {
      setLoading(true);
      try {
        const { data } = await apiPublic.post("/api/auth/google", {
          credential,
        });
        console.log(
          "Respuesta del backend tras autenticación con Google:",
          data.user,
        );
        ...
        if (backendEmail) {
          try {
            const roleInfo = await resolveRoleFromUsersSheet(
              backendEmail,
              token,
            );
